...
- 项目初始化、工程搭建和核心能力实现优先采用主流框架、官方脚手架和成熟组件，不使用偏门方案，也不以纯手工逐文件搭建代替标准初始化流程。
...
- 前端不是本项目的零基础学习主线，但仍然属于正式交付范围；需要完成页面实现、接口联调，并说明关键实现思路和前后端数据流。
...
| 前端请求 | 原生 Fetch 或现有团队请求库 | 由前端按习惯选择；前端需要正常实现并与后端联调，但学习重点仍放在后端 |
